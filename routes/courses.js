...
Router
    ...
    .post('/all', verifyAdmin, async (req, res) => {
        try {
    ...
    .get('/:id', verifyAdmin, async (req, res) => {
        try {
    ...
    .put('/:id', [
        ...
        // Checking if atleast 1 field is present
        if (Object.keys(req.body).length === 0) return res.status(400).send("Atleast 1 field is required");

        try {

            // Checking if the course exists
            const course = await pool.query("SELECT * FROM courses WHERE id=$1", [req.params.id]);
            if (course.rows.length === 0) return res.status(404).send("No course exists with this id");
            
            // Query for updating the course according to the fields provided
    ...
    .delete('/:id', verifyAdmin, async (req, res) => {
        try {
            // Checking if the course exists
            const course = await pool.query("SELECT * FROM courses WHERE id=$1", [req.params.id]);
            if (course.rows.length === 0) return res.status(404).send("No course exists with this id");
